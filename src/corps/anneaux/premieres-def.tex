...
\section{Algèbres de polynômes}\label{Poly}\index{Polynômes}
...
i+j=n}a_ib_j)e_n\end{equation} % avec un environnement equation, la
numérotation se fait automatiquement

On vérifie facilement que $(A^{(\N)},+,*)$ est un anneau commutatif ayant pour
...
\begin{lemme}[Propriété universelle de la $A$-algèbre des polynômes à une
indéterminée]
...
On peut aussi réécrire \ref{Poly}.3 en disant que, pour toute $A$-algèbre
...
\begin{remarque}
    \phantomsection\label{RemarqueConstr} Ce qui nous a permis de définir le
...
\begin{lemme}[Propriété universelle de la $A$-algèbre du monoïde $(N,\cdot)$]
    Pour tout anneau commutatif $A$, il existe une $A$-algèbre $\iota_A:
...
% ,\cdot)$ revient à se donner l'image $b\in B$ de $1\in \N$).\\

Soit $(N,\cdot)$ un monoïde et $\phi: A\rightarrow B$ un morphisme d'anneaux
...
\begin{definition}
    ...
    fini\index{de type fini (Algèbre)}.
\end{definition}
