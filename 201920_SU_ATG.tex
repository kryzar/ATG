...
\xymatrixrowsep{1.8cm}\xymatrixcolsep{1.8cm}
...
\begin{remarque}On peut aussi réécrire \ref{ProduitSommeDirecte} en disant que, pour tout $A$-module $M$ les morphismes canoniques
...
La propriété universelle de $\iota_x:A\hookrightarrow A^{(X)}$, $x\in X$ appliquée aux morphismes  de $A$-modules $-\dot x: A\rightarrow M$, $x\in X$ nous donne un unique morphisme de $A$-modules  $p_X:A^{(X)}\rightarrow M$ tel que $p\circ \iota_x(a)=ax$, $x\in X$. On vérifie immédiatement que les propriétés suivantes sont équivalentes :
\begin{enumerate}
...
est exacte\index{Exacte (Suite de morphismes)} si $\im(u_{i})=\ker(u_{i+1})$ pour tout $0\leq i\leq n-1$.
\end{definition}
...
\begin{remarque}La notion de suite exacte est au coeur de l'étude de la structure des $A$-module. La raison première est que c'est l'outil qui permet de 'dévisser' un $A$-module compliqué ($M$) en deux $A$-modules plus simples ($M'$ et $M''$).\end{remarque}
